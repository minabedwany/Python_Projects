...
def handle_commands(C: list) -> list:
    ...
    while True:
        ...
        if response=="q":
            ...
            n = float(input("Please enter the percentage: "))
...
def Restaurant_change_price(R: Restaurant, num: float) -> Restaurant:
    """Changes the price for a restaurant in the collection
    """
    R = R._replace(price = R.price + R.price*(num/100))
    return R
...
def Collection_change_prices(C: list, num: float) -> list:
    """Changes the price for all restaurants in the collection
    """
    x=[]
    for i in C:
        x.append(Restaurant_change_price(i, num))
    return x
